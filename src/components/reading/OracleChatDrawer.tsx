...
export const OracleChatDrawer: React.FC<OracleChatDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-[#0c0a09] border-l border-[#292524] shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
      
      {/* Header */}
      <div className="p-4 sm:p-5 border-b border-[#292524] flex items-center justify-between bg-[#141210]">
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 flex-shrink-0 flex items-center justify-center bg-[#0c0a09] border border-[#292524] rounded-lg">
            <Sparkles className="w-5 h-5 text-[#f5f5f4]" />
          </div>
          <div>
            <h3 className="font-serif font-bold text-sm sm:text-base text-[#f5f5f4]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5">
        {messages.map((m) => (
          ...
          >
            <div
              className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 text-xs border ${
                m.sender === 'oracle'
                  ? 'bg-[#141210] border-[#292524] text-[#f5f5f4]'
                  : 'bg-[#1c1917] border-[#78716c] text-[#a8a29e]'
              }`}
            >
            ...
            <div
              className={`max-w-[82%] rounded-xl px-3.5 py-2.5 text-xs sm:text-sm font-sans leading-relaxed ${
                m.sender === 'oracle'
                  ? 'bg-[#141210] border border-[#292524] text-[#f5f5f4]'
        ...
        {isTyping && (
          <div className="flex items-center space-x-2 text-xs text-[#78716c] font-sans italic pl-9">
      ...
      <div className="p-4 border-t border-[#292524] bg-[#141210] space-y-2 pb-[calc(1rem+env(safe-area-inset-bottom,0px))]">
        <div className="text-[10px] font-mono text-[#78716c] uppercase tracking-wider px-0.5 flex items-center space-x-1.5">
        ...
        <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto pt-1">
          {quickQuestions.map((q, idx) => (
            <button
              ...
              className="text-xs font-sans text-[#a8a29e] hover:text-[#f5f5f4] bg-[#0c0a09] hover:bg-[#1c1917] border border-[#292524] hover:border-[#78716c] px-3 py-1.5 rounded-lg transition-all text-left flex items-center space-x-1.5"
            >
